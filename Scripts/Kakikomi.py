...
def canny_image(image):
    ...
    # 画像の輪郭を抽出
    output = cv2.Canny(gray_image, 0, 100)  # 輪郭の強度の閾値を調整可能

    return Image.fromarray(output)  # PILイメージに変換して返す
...
def load_pipeline(vae, control_net_canny):
    return StableDiffusionControlNetImg2ImgPipeline.from_pretrained(
...
def main(init_image, mask_image, mode, max_size,prompt, strength):
    ...
    if mode == "MaskON":
        vae = load_vae_autoencoder()
        pipe = load_pipeline(vae, control_net_canny).to(CUDA_DEVICE) 
        ...
        pipe.load_textual_inversion("Models/textual_inversion/ParaNegative", weight_name="ParaNegative.safetensors", token="ParaNegative", torch_dtype=torch.float16)
        ...
        negative_prompt = "Mayng, ParaNegative, (low quality, worst quality:1.4)"
        ...
        control_image = canny_image(resized_image)
...
if __name__ == "__main__":
    ...
    max_size = 960
    strength = 0.4
    prompt = "1girl, solo, looking_at_viewer, smile, blonde_hair, simple_background, long_sleeves, white_background, dress, green_eyes, standing, full_body, boots, belt, cape, instrument, harp"
    image = main(init_image, mask_image, mode, max_size,prompt,strength)
    image.save("Secta_diffuserss_960.png")

    #image = Image.open(image_path).convert("RGB")
    #canny_image = canny_image(image)
